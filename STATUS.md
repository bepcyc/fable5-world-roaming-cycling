...
- **ВЕРДИКТ ВЛАДЕЛЬЦА (главное).** Дословно: «у тебя один вел из трёх и
  ...
  несущественны» — решил за владельца; планка его). Кокпит-REWORK по
  референсу `shots/wip/what_i_want/Pasted image.png` — кандидат №1 на
  сессию 9: тонкий руль, читаемые капюшоны манеток, митенки с анатомией
  пальцев, едва заметные тросы, покрышка+тень; судить только кадрами
  бок-о-бок с референсом.
