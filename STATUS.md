...
## Session 9 — 2026-07-03: M1.5.2 кокпит-REWORK (итерации 1–5, /loop владельца)

Владелец запустил /loop: «постоянно улучшай вид из глаз всех трёх велов,
side-by-side с референсом в бот, разбор ОЖИДАЛ/ВИЖУ — священен». По ходу
уточнил: (а) «модели самих велов — мусор, работай над ними»; (б)
«перспективу сохрани НАШУ — мы смотрим как смотрим, но вел в кадре должен
быть виден больше». Камеру НЕ трогал (pitch −0.06 остался); вел приближен
посадкой: BAR_DROP 0.30→0.165, BAR_FWD 0.50→0.40.

- **Комп** (`ComputerScreen.ts` переписан): портретный телефонный формат
  на стойке, раскладка референса пиксель-в-пиксель — SPEED крупно /
  POWER|CADENCE / HEART RATE с красной полоской | DISTANCE / TIMER
  hh:mm:ss (новый ride-таймер с автопаузой в Cockpit.ts) + статус-ряд и
  точки страниц; heartRateBpm подключён; карта Garmin-style — страница
  `?ckmap=1`; hazard P5 = красный баннер сверху экрана.
- **Руки v2** (`CockpitParts.ts` переписан): анатомия — ладонь-подушка с
  куполом, ряд костяшек, 4 плотных пальца гладкими дугами (TubeGeometry
  по суставным точкам; капсульные цепочки читались гусеницами), большой
  палец с тенаром подвёрнут, конусные предплечья в нижние углы кадра,
  браслет-шнурок на правом запястье (road/gravel), полные перчатки с
  манжетами/падами и вытянутым указательным на рычаг (MTB). Анатомия
  хвата: фаланга-1 почти плоско ПОПЕРЁК верха трубы, суставы
  подворачивают кончики под неё.
- **Велы по референсам**: road = аэро-крыло (расплющенный/уширенный свип)
  + интегрированный вынос, капюшоны выведены вперёд из-под крыла, тросов
  НОЛЬ; gravel = круглый топ + флейр 13°, чёрная обмотка, ОДИН тонкий
  тросик у рулевой; MTB = riser 780 мм, lock-on грипсы, клампы+клинки
  рычагов+поды+гидролинии, корона с ногами и цветными регулировками
  (оранж L / син R). РАМА появилась: рулевая+верхняя+низ труб под
  выносом — белый перламутр (road/gravel), олива (MTB); знак z у
  STEER_AXIS исправлен (вилка клонилась В райдера). Одно общее колесо.
- **Motion blur покрытия** (ядро «ощущения скорости» референсов):
  пост-пасс между TRAA-resolve и bloom — 7 тапов вдоль пиксельной
  камерной репроекции (переиспользован velReproject), кокпит исключён по
  глубине (вел резкий, обочины мажутся), сила = cockpitVelU.mblur от
  скорости рига (полная к ~36 км/ч, 0 на месте/walk/fly). Рубильники
  ?mblur=0 / ?ablate=mblur. fps не просел (~48 @1600×900).
- **?dash=0** (RideHud) — чистые судейские кадры без DOM-карточек.
- **probe-cktraa**: маска luma 80→95 (новый кокпит светлее — голая кожа);
  ALL PASS до и после блюра (ratio 16–24×); probe-menu-ride, probe-hudwarn
  зелёные; tsc чист. Коммиты: 01fd751, e8b5490, b98bcd5 (+ этот).
- В бот отправлено 20+ кадров с разбором ОЖИДАЛ/ВИЖУ: sbs всех трёх
  режимов ×3 итерации + A/B блюра (shots/wip/m152/, вне git по .gitignore).
- Открыто по sbs: капюшонный хват менее «обнимающий» чем референс;
  белая труба road скромнее референсной (стек крыло/комп закрывает);
  MTB корона с цветными регулировками ниже кромки при нашей перспективе
  (компромисс, перспектива владельца); кожа без микрорельефа; MTB sbs со
  спуска (смаз зрелищнее) не снят — spawn едет в подъём.

**Session 9 closed (owner stopped the /loop), потом /loop перезапущен и
остановлен снова после итераций 6–7 (session 9b, 2026-07-04): ck-shot
тул (setMode ДО teleport!), кожа с вариацией+темнее, руки мясистее,
хват капюшонов глубже, судейские точки на скорости найдены (asphalt@0.1
прямая 27 км/ч — v14b снят; gravel-coarse@0.15 и singletrack@0.65 —
на следующую сессию). Коммиты f2d7db5, 39422f3. Next session: read
`.claude/handoffs/2026-07-04-session-9b-m152-iter6-7.md` (+ session-9
handoff). Вердикта владельца по итерациям 1–7 НЕТ — M1.5.2 открыт до
его «принято». ВЕРДИКТ ПО РУКАМ (2026-07-04): «наложены на руль» —
оформлено как **M1.5.3** (ROADMAP + handoff 9b): кисть единой сеткой по
