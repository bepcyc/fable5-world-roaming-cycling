...
| Water | `src/world/WaterSurface.ts`, `src/render/WaterMaterial.ts`, `src/gpu/passes/Caustics.ts` | 6-level camera clipmap over hydrology fields; SSR, absorption, foam, caustics |
...
- Per-texel classification storage exists to extend: `biomeTex` 4096² rgba8 = biomeId/8, snow, vegDensity, rockExposure (`src/gpu/passes/BiomeSnow.ts:44-53`, ids in `src/world/WorldConst.ts:28-45`); hydrology fields in `fieldsTex` 2048² rgba16f (`src/world/Heightfield.ts:424`).
