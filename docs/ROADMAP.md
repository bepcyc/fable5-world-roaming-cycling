...
- **Scope:** `BleSensorSource` implementing the seam: FTMS (indoor bike data + control point), cycling power (0x1818), CSC (0x1816), HR (0x180D); connect UI (user-gesture requirement), device persistence, dropout/reconnect handling; the no-sensor gate (bike modes locked without a live power source; demo mode stays badged); FTMS SIM-gradient resistance if the open question lands in MVP.
- **Acceptance:** probe **P6 sensor-dropout** (synthetic transport faked under the BLE adapter interface) — coast to natural stop, reconnect UI, no crash; **P7 demo-badge**; **real-hardware exit criterion:** one full ride on the owner's trainer + HR strap on the play machine, values sane vs head unit.
- **Depends on:** M1.3 (physics consumes power). **Sessions:** 1–2. **Top risk:** hardware variance (FTMS quirks per trainer brand) + no BT adapter on the dev box — all dev happens against the seam; schedule the real-device test with the owner early, not at the end.
...
- **M3.2 FTMS SIM resistance** (if not landed in M1.4) + **treadmill FTMS for hiking**: incline-driven walk speed, treadmill pace → hike pace. 1–2 sessions.
